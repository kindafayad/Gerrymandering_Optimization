...
# Preprocessing the population data to select the most recent year
most_recent_year_population_data = population_data[population_data['Year'] == population_data['Year'].max()]
simplified_population_data = most_recent_year_population_data[['GEOID', 'Total']].copy()
...
simplified_population_data.reset_index(drop=True, inplace=True)  # Ensure alignment
...
d = 37  # Specify the number of districts
...
for k in range(d):
    for i in range(n):
        neighbors = [j for j in range(n) if adjacency_matrix_aligned.iloc[i, j] == 1]
    ...
    for i in range(n):
        for j in range(n):
            if i != j and A[i, j] == 0:  # Non-adjacent counties
                model.addConstr(x[i, k] + x[j, k] <= 1, f"non_adj_{i}_{j}_{k}")
    ...
    for i in range(n):
        for j in range(d):
            if x[i, j].X > 0.5:  # If county i is assigned to district j
                ...
                county_name = most_recent_year_population_data[most_recent_year_population_data['GEOID'] == geo_id]['County'].values[0]
